...
for s = 1:depthTree
    ...
    if (search)
        ...
        parents = [];
        parfor i=first:last
            %tempStorage = zeros(maxBranching*nPrimitives,10);
            %tempParents = zeros(maxBranching*nPrimitives,1);
            tempStorage = [];
            tempParents = [];
            for j=1:nPrimitives
            
              %ind1 = i + 1- first;
              %ind2 = nPrimitives*(ind1 - 1) + j; 
            
               newNode = checkConstraints(graph.verts(i,:),motionPrimitiveArray(j,:)', deltaT, tauLimit, jointLimitQ, active_joints);

               if (newNode ~= 9999)
                   %tempStorage(ind2,:) = newNode;
                   %tempParents(ind2) = i;
                   
                   %tempStorage(ind2,:) = newNode;
                   %tempParents(ind2) = i;
                    
                   tempStorage(end +1,:) = newNode;
                   tempParents(end+1) = i;
                   
               end
            end
            storage = [storage; tempStorage];
            parents = [parents, tempParents];
        end
        ...
        for i=1:size(storage,1)
            ...
            if (dist <= threshold)
                ...
                graph.solutionNodes(end +1) = index;
